...
const fs = require('fs');
const Pharmacist = require('../../../../../models/user_models/pharmacist_model'); // Assuming your model file is in the specified path
...
exports.updatePharmacistProfileImage = async (req, res, user) => {
  try {
    ...
    upload(req, res, async function (err) {
      ...
      if (req.file) {
        ...
        if (patient.avatarSrc) {
          fs.unlinkSync(pharmacist.avatarSrc);
        }
        // Update profileImage field with new path
        pharmacist.avatarSrc = req.file.path;
      }
