...
exports.uploadPatientProfileImage = async (req, res) => { 
  try {
    const { id } = req.params;
    ...
    const storage = multer.diskStorage({
      ...
      filename: async function (req, file, cb) {
        const existingFilePath = patient.avatarSrc ? path.join(__dirname, '../../../../../', patient.avatarSrc) : null;
   ...
   const avatarSrcPath = req.file.filename
      ...
      // Update patient's avatarSrc field with the file path
      patient.avatarSrc = avatarSrcPath;

      // Save changes to the database
      await patient.save();

      console.log("Updated patient's avatarSrc")
