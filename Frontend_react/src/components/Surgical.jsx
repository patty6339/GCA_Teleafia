...
function Surgical() {
  ...
  useEffect(() => {
    const fetchDescription = async () => {
      try {
        ...
        const surgicalService = data.services.find(service => service.name.toLowerCase() === 'surgical');
        if (surgicalService) {
          setDescription(surgicalService.description);
        } else {
  ...
  return (
    <Box sx={{ bgcolor: '#FCF4F4', width: '850px', marginLeft: '30px', marginTop: '20px', borderRadius: '20px', height: '1000vh' }}>
      ...
      <h4 style={{ marginLeft: '70px', color:'#C00000' }}>Surgical</h4>
